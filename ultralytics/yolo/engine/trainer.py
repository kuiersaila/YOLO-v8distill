...
class NmimicLoss(nn.Module):
    def __init__(self, channels_s, channels_t):
        super(NmimicLoss, self).__init__()
        self.mse = nn.MSELoss()
    ...
    def forward(self, y_s, y_t):
        ...
        for idx, (s, t) in enumerate(zip(y_s, y_t)):
            ...
            # 按元素个数归一化损失
            normalized_loss = mse_loss / (s.numel())
...
class FeatureLoss(nn.Module):
    def __init__(self, channels_s, channels_t, distiller='nmi', loss_weight=1.0):
        ...
        if distiller == 'mgd':
            ...
            self.feature_loss = NmimicLoss(channels_s, channels_t)
        else:
